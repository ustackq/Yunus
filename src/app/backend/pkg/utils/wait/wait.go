...
import (
	...
	"github.com/ustack/Yunus/src/app/backend/pkg/runtime"
)
...
// SatrtWithChannel starts f in a new goroutine in the group.
// stopCh is paased to f as an argument. f should stop when stopCh is available.
func (g *Group) SatrtWithChannel(stopCh <-chan struct{}, f func(stopCh <-chan struct{})) {
	g.Start(func() {
		f(stopCh)
	})
}
...
// Until loops util stop channel is closed, running f every period.
...
var ErrWaitTimeout = errors.New("time out waiting for the condition")

// ConditionFunc returns true if the condition is satisfied, or an error
// if the loo should be aborted.
